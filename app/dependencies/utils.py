...
async def replace_pad(pad_code: list[str], template_id: int) -> dict[str, str]:
    ...
    replace_pad_body = {"realPhoneTemplateId": template_id, "padCodes": pad_code}

    return await VmosUtil(pad_infos_url, replace_pad_body).send()


async def update_language(
    language: str, country, pad_code_list: list[str]
) -> dict[str, str]:
...
async def install_app(
    pad_code_list: list[str], app_url: str, md5: str
) -> dict[str, str]:
...
async def get_pad_info(pad_code):
    ...
    pad_info_data = {"padCode": pad_code}
...
async def exe_cmd(pad_code: str, cmd: str):
    ...
    json = {"padCode": pad_code, "scriptContent": cmd}
...
class Position:
    ...
    def to_dict(self) -> dict:
        result = {"actionType": self.action_type.value, "x": self.x, "y": self.y}
...
async def update_time_zone(pad_code_list: list[str], time_zone: str) -> dict[str, str]:
    ...
    body = {"timeZone": time_zone, "padCodes": pad_code_list}

    return await VmosUtil(update_timezone_url, body).send()


async def gps_in_ject_info(
    pad_code_list: list[str], longitude: float, latitude: float
) -> dict[str, str]:
    set_local_url = "/vcpcloud/api/padApi/gpsInjectInfo"

    body = {"longitude": longitude, "latitude": latitude, "padCodes": pad_code_list}
