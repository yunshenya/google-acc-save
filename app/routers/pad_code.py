...
async def get_available_pad_codes(_: str = Depends(verify_token)) -> Dict[str, Any]:
    ...
    try:
        ...
        if not response or response.get("code") != 200:
            ...
            raise HTTPException(
                status_code=500, detail=f"获取云端pad_code失败: {error_msg}"
            )
            ...
            formatted_data.append(
                {
                    ...
                    "isInConfig": pad.get("padCode")
                    in Config.PAD_CODES,  # 是否已在配置中
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"获取可用pad_code列表失败: {str(e)}"
        )
...
async def get_current_pad_codes(_: str = Depends(verify_token)) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "data": current_codes, "total": len(current_codes)}
...
async def sync_pad_codes(
    sync_request: PadCodeSyncRequest, _: str = Depends(verify_token)
) -> PadCodeSyncResponse:
    """同步 pad_code 到配置中"""
    try:
        logger.info(
            f"开始同步pad_code，选择的代码数量: {len(sync_request.selected_codes)}"
        )
        ...
        available_codes = {
            pad.get("padCode") for pad in available_pads if pad.get("padCode")
        }
        ...
        # 使用配置更新方法
        Config.update_config({"pad_codes": new_pad_codes})

        logger.info(
            f"同步完成 - 新增: {len(added_codes)}, 已存在: {len(updated_codes)}"
        )
...
async def add_pad_codes(
    ...
    try:
        ...
        if not response or response.get("code") != 200:
            error_msg = response.get("msg", "未知错误") if response else "API调用失败"
            raise HTTPException(
                status_code=500, detail=f"验证pad_code失败: {error_msg}"
            )
        ...
        Config.update_config({"pad_codes": updated_codes})
...
async def remove_pad_codes(
    ...
    try:
        ...
        if not actually_removed:
            return {"success": "true", "message": "指定的代码都不在当前配置中"}
...
async def replace_pad_codes(
    ...
    try:
        ...
        # 完全替换配置
        Config.update_config({"pad_codes": sync_request.selected_codes})

        logger.warning(
            f"已完全替换pad_code配置，从 {old_count} 个变为 {len(sync_request.selected_codes)} 个"
        )
