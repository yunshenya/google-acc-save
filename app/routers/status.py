...
async def update_status_server(status_request: StatusRequest) -> StatusRequest:
...
async def get_one_cloud_status(one_cloud_status_request: GetOneCloudStatus) -> StatusRequest:
     async with SessionLocal() as db:
          from sqlalchemy import select
          result = await db.execute(select(Status).filter(cast(ColumnElement[bool], Status.pad_code == one_cloud_status_request.pad_code)))
