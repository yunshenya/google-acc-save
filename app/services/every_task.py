...
async def start_app_state(package_name, pad_code, task_manager):
    ...
    try:
        while total_try_count < 6:
            app_result: Any = await start_app(
                pad_code_list=[pad_code], pkg_name=package_name
            )
            ...
            match await check_padTaskDetail([taskid]):
                case -1:
                    ...
                    await update_cloud_status(
                        pad_code=pad_code, current_status="启动任务正在一键新机"
                    )
                ...
                case 0:
                    ...
                    await update_cloud_status(
                        pad_code=pad_code, current_status="启动app中..."
                    )
                    await asyncio.sleep(2)

                case 1:
                    await update_cloud_status(
                        pad_code=pad_code, current_status="开始点击脚本"
                    )
                    await sleep(5)
                    await click(
                        [pad_code],
                        [
                            ...
                            Position(
                                x=456, y=674, action_type=ActionType.lift
                            ).to_dict(),
                        ],
                    )
                    await sleep(5)
                    ...
                    await update_cloud_status(
                        pad_code=pad_code, current_status="app启动成功"
                    )
                    break
            total_try_count += 1
        ...
        while total_try_count < 6:
            ...
            match await check_padTaskDetail([taskid]):
                ...
                case 1:
                    logger.success(f"{pad_code}: 启动app成功")
                    await update_cloud_status(
                        pad_code=pad_code, current_status="启动app成功"
                    )
                    break
            total_try_count += 1
...
async def install_app_main_logic(pad_code_str: str, task_manager):
    ...
    await update_cloud_status(
        pad_code=pad_code_str, current_status="一键新机成功，开始安装应用"
    )
